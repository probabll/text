...
class Vocabulary:
    """
    Maintains a map from strings to integers and back.
    """
    ...
    def print_statistics(self, stdout=sys.stdout):
        print(f"Vocabulary size: {len(self.word_to_idx):,} word types", file=stdout)
        print(file=stdout)

        if len(self.word_freqs) > 0:

            # Print the most frequent words.
            print("Top 10 most frequent words:", file=stdout)
            for word, freq in sorted(self.word_freqs.items(), key=lambda kv: kv[1],
                                     reverse=True)[:10]:
                print(f"{' '*3}{word:30}frequency = {freq:,}", file=stdout)
            print(file=stdout)

            # Print the least frequent words.
            print("Top 10 least frequent words:", file=stdout)
            for word, freq in sorted(self.word_freqs.items(), key=lambda kv: kv[1])[:10]:
                print(f"{' '*3}{word:30}frequency = {freq:,}", file=stdout)
            print(file=stdout)

        # Print the longest words.
        print("Top 10 longest words:", file=stdout)
        for word in sorted(self.word_to_idx.keys(), key=lambda w: len(w), reverse=True)[:10]:
            if word in self.word_freqs:
                ...
                print(f"{' '*3}{word:30}length = {len(word):<5}frequency = {freq:,}", file=stdout)
            else:
                print(f"{' '*3}{word:30}length = {len(word):<5}", file=stdout)
        print(file=stdout)

        # Print the shortest words.
        print("Top 10 shortest words:", file=stdout)
    ...
    @staticmethod
    def from_data(filenames: list, min_freq=0, max_size=-1):
        """
    ...
    @staticmethod
    def from_file(vocab_file: str, max_size=-1):
        """
